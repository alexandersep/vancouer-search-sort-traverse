...
import java.util.*; // Scanner, HashMaps, Lists
import java.io.*;
...
public class CompetitionDijkstra {
    ...
    CompetitionDijkstra (String filename, int sA, int sB, int sC){
        ...
        try {
            ...
            G = new EdgeWeightedDigraph.Digraph(numIntsec, numStreets); // created new EdgeWeight Digraph
            ...
            while(input.hasNextLine()) {
                try {
                    ...
                    G.addEdge(currentStreet, newStreet, weight);
                } catch (Exception ex) {
                    return;
                }
            }
    ...
    public int timeRequiredforCompetition(){
        ...
        int N = G.v;
        for(int i = 0; i < N; i++) {
            DijkstraSP dijkstrasp = new DijkstraSP(G, i); // shortest path graph dijkstra (using mapped matrix and using dijkstra) after G check null
            for(int j = 0; j < N; j++) {
                tempDist = dijkstrasp.distTo(j);
